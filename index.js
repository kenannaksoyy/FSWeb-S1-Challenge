...
function indekseGoreFenomen(gelen_fenomenler,indeks) {
  let format_donus=`${indeks}. indekste bulunan fenomen: ${gelen_fenomenler[indeks].profile}`;
  return format_donus;
...
function profilListesi(gelen_fenomenler) {
  let kopya_fenomenler=gelen_fenomenler;
  let dizi=[];
  for (let i=0;i<kopya_fenomenler.length;i++){
    dizi.push(kopya_fenomenler[i].profile);
  }
  return dizi;
}
...
function fenomenSil(gelen_fenomenler,indeks) {
  let kopya_fenomenler=gelen_fenomenler;
  kopya_fenomenler.splice(indeks,1);
  return kopya_fenomenler;
...
function fenomenEkle(fenomenler,number,profile,followers,posts,platform) {
  let kopya_fenomenler=fenomenler;
  const yeni_nesne=
  {
    number: number,
    profile: profile,
  ...
  kopya_fenomenler.push(yeni_nesne);
  return kopya_fenomenler;
}
...
function enFenomenler(gelen_fenomenler) {
  const f_sinir=100000000;
  let dizi=[]
  for(let i=0;i<gelen_fenomenler.length;i++){
    if(gelen_fenomenler[i].followers>f_sinir){
      dizi.push(gelen_fenomenler[i].profile);
    }
  }
  return dizi;
}
...
function fenomenGonderimSayisi(gelen_fenomenler,gelen_profil){
  let gonderim_sayisi;
  for(let i=0;i<gelen_fenomenler.length;i++){
    if(gelen_fenomenler[i].profile===gelen_profil){
      gonderim_sayisi=gelen_fenomenler[i].posts;
    }
  }
  return gonderim_sayisi;
}
...
function platformaGoreCokGonderiYapanFenomen(gelen_fenomenler,gelen_platform){
  let tut=0;
  let tut2;
  for(let i = 0;i<gelen_fenomenler.length;i++){
    if(gelen_fenomenler[i].platform===gelen_platform && gelen_fenomenler[i].posts !=="NA"){
      if(gelen_fenomenler[i].posts>tut){
        tut2=i;
        tut=gelen_fenomenler[i].posts;
      }
    }
  }
  return gelen_fenomenler[tut2].profile;
}
